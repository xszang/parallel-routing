...
import com.xilinx.rapidwright.rwroute.PartitionTree;
import java.util.concurrent.locks.Lock;
import java.util.concurrent.locks.ReentrantReadWriteLock;
import java.util.concurrent.locks.ReentrantLock;
/**
 * RWRouteParallel class provides the main methods for routing a design.
 * Creating a RWRouteParallel Object needs a {@link Design} Object and a {@link RWRouteConfig} Object.
 */
public class RWRouteParallel{
    /** The design to route */
    ...
    protected RuntimeTrackerTree routerTimer; // TODO: check the sync of the following timer
    ...
    protected RouteNodeGraph routingGraph; // TODO: check the sync of the routingGraph 
	...
	private int connectionIdBase; // for mult-thread
    /** Total number of nodes pushed/popped from the queue */
    // private long nodesPushed; // disable for multi-thread; to support if needed 
    // private long nodesPopped; // disable for multi-thread; to support if needed 
    ...
    public RWRouteParallel(Design design, RWRouteConfig config) {
        this.design = design;
        this.config = config;
    }

    protected static String getUnsupportedSeriesMessage(Part part) {
        return "ERROR: RWRouteParallel does not support routing the " + part.getName() + " from the " 
    ...
    protected NodeStatus getGlobalRoutingNodeStatus(Net net, Node node) {
        if (routingGraph.isPreserved(node)) {
            // Node is preserved by any net -- for base RWRouteParallel, we don't need
	...
	private void routePartitionTree(PartitionTreeNode node) {
		...
		ParallelismTools.joinBlockingQueue(jobs);
	}
    ...
    protected void postRouteProcess() {
        ...
        if (lutRoutethru) {
            // It is possible for RWRouteParallel to routethru a LUT that is already being used as a
    ...
    /**
     * Updates the congestion cost factors for general designs.
     */
        ...
        while ((rnode = queue.poll()) != null) {
            ...
            if (rnode.isTarget()) {
                if (rnode != connection.getSinkRnode())
                    System.out.println("Conn " + connection.hashCode() + " find a different target: " + rnode.toString_() + " , Conn Sink: " + connection.getSinkRnode().toString_());
                break;
            }
    ...
    private boolean saveRouting(Connection connection, RouteNode rnode) {
        ...
        if (rnode != sinkRnode && !altSinkRnodes.contains(rnode)) {
            ...
            if (!connection.getSink().isRouted() || prevRouting.isEmpty() || !rnode.isTarget()) {
                throw new RuntimeException("Unexpected rnode to backtrack from: " + rnode + " isTarget " + rnode.isTarget());
            }
        ...
        if (rnodes.size() == 1) {
            ...
            // System.out.println("Conn " + connection.hashCode() + " No prev pointer from sink rnode -> not routed");
            return false;
        }
        ...
        if (!sourceRnode.equals(connection.getSourceRnode())) {
            if (!sourceRnode.equals(connection.getAltSourceRnode())) {
                ...
                System.out.println("Conn " + connection.hashCode() + " find a different source " + sourceRnode.toString_() + " conn source: " + connection.getSourceRnode().toString_() + " conn alt source: " + connection.getAltSourceRnode().toString_());
                return false;
            }
    ...
    private static Design routeDesign(Design design, RWRouteConfig config) {
        ...
        return routeDesign(design, new RWRouteParallel(design, config));
    }

    /**
     * Routes a design after pre-processing.
     * @param design The {@link Design} instance to be routed.
     * @param router A {@link RWRouteParallel} object to be used to route the design.
     */
    protected static Design routeDesign(Design design, RWRouteParallel router) {
        router.preprocess();
    ...
    /**
     * The main interface of {@link RWRouteParallel} that reads in a {@link Design} design
    ...
    public static void main(String[] args) {
        ...
        CodePerfTracker t = new CodePerfTracker("RWRouteParallel", true);

...
%_$(ROUTER).check: %.netlist %_$(ROUTER).phys %_unrouted.phys | compile-java
	if java $(JVM_HEAP) -jar build/libs/main.jar CheckPhysNetlist $^ $(call log_and_or_display,$@.log); then \
        echo "PASS" > $@; \
    else \
        echo "FAIL" > $@; \
    fi
...
%_cufr.phys: %_unrouted.phys | compile-java
	# (/usr/bin/time ./RapidWright/bin/rapidwright 4 PartialRouterPhysNetlist $< $@) $(call log_and_or_display,$@.log)
	(/usr/bin/time java -XX:ActiveProcessorCount=$(THREAD) -Xss1g -jar build/libs/main.jar PartialRouterPhysNetlist $< $@) $(call log_and_or_display,$@.log)
